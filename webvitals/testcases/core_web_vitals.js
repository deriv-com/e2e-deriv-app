...
async function getTestURLs() {
  for (const row of xl_data) {
    ...
    if (url === 'https://staging-app.deriv.com') {
      ...
      url.startsWith('https://staging-app.deriv.com/') &&
      url.length > 'https://staging-app.deriv.com/'.length
    ) {
...
async function getPerformanceMetrics(session_url, device) {
  try {
    ...
    output_file = `../report_outputs/${testId}_${device}_${session_url
      .replace(/^https?:\/\//, '')
      .split('?')[0]
      .replace(/[^\w\s]/gi, '_')}.json`
    ...
    const scores = {
      ...
      LCP: testData.data.average.firstView[
        'chromeUserTiming.LargestContentfulPaint'
      ],
      CLS: testData.data.average.firstView[
    ...
    if (
      ...
      scores.LCP === null ||
      scores.CLS === null ||
      scores.PAGE_FULLY_LOADED === null ||
      scores.FULL_REPORT === null
    ) {
...
function compareScores(prodStagingScores, tagStagingScores, device, url) {
  ...
  validateMetric(
    'FCP',
    prodStagingScores.FCP,
    tagStagingScores.FCP,
    FCPThreshold,
    tagStagingScores.FULL_REPORT,
    url,
    device
  )
  validateMetric(
    'LCP',
    prodStagingScores.LCP,
    tagStagingScores.LCP,
    LCPThreshold,
    tagStagingScores.FULL_REPORT,
    url,
    device
  ...
  validateMetric(
    'CLS',
    prodStagingScores.CLS,
    tagStagingScores.CLS,
    CLSThreshold,
    tagStagingScores.FULL_REPORT,
    url,
    device
...
function validateMetric(
  ...
  if (stagingValue <= productionValue) {
    console.log(
      `Staging ${metricName} score is better than or same as production score.`
    )
    if (stagingValue < productionValue) {
    ...
    if (stagingValue <= threshold) {
      console.log(
        `Staging has higher ${metricName} score compared to production but within 3% threshold.`
      )
    } else {
      console.error(
        `Staging has higher ${metricName} score exceeding 3% threshold.`
      )
      failedTests.push(
        `Staging test: ${stageFinalReport} failed for ${metricName} against prod`
      )
    }
  }
}
