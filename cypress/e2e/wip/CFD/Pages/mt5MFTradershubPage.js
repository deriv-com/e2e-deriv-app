...
class mt5_tradershub {
  ...
  verifyText = (webelement, expectedText) => {
    cy.get(webelement).then(($element) => {
  ...
  verifyMT5DemoPasswordModal = () => {
    ...
    let mt5Success
    let passwordgen
    cy.get(mt5locators.mt5Locators.mt5PasswordModelTitle)
      .invoke('text')
      .then((text) => {
        mt5PasswordTitle = text.trim()
        cy.log('MT5 DEMO Title:: ' + mt5PasswordTitle)
        expect(mt5PasswordTitle).to.equal(
          textVal.textValidation.mt5CreatePasswordMsg
        )
      })
    passwordgen = this.generateRandomString()
    this.inputText(mt5locators.mt5Locators.mt5InputPassword, passwordgen)
    cy.wait(1000)
    cy.get(mt5locators.mt5Locators.mt5ButtonCreatePassword)
      .should('be.visible')
      .click()
    cy.wait(2000)
    ...
    cy.get('.dc-modal-body')
      ...
      .then((text) => {
        ...
        expect(mt5Success).to.equal(
          textVal.textValidation.mt5DemoAccountSuccess
        )
      })
    cy.get(mt5locators.mt5Locators.mt5ButtonContinue)
      .should('be.visible')
      .click()
  }
  createMT5MFDemo = () => {
    cy.get(mt5locators.mt5Locators.mt5ButtonCfdDemo)
      .should('be.visible')
      .click()
  ...
  verifyDemoPageLanding = () => {
    cy.get(mt5locators.mt5Locators.mt5TextAccountType).should(
      'have.text',
      'demo'
    )
  }
  verifyMDemoAccountDetailsValidation = () => {
    ...
    cy.get(mt5locators.mt5Locators.mt5TradingDetails)
      ...
      .then((text) => {
        ...
        expect(demoCurrency.trim()).to.equal(
          textVal.textValidation.mt5EURDemoBalance
        )
      })
  }
  generateRandomString = () => {
    ...
    randomUpper =
      ...
      capitalLetters.charAt(
        crypto.getRandomValues(new Uint32Array(1))[0] % capitalLetters.length
      )
    digit =
      digit +
      digits.charAt(
    ...
    for (let i = 0; i < 6; i++) {
      randomLower =
        randomLower +
        smallLetters.charAt(
          crypto.getRandomValues(new Uint32Array(1))[0] % smallLetters.length
        )
    }
