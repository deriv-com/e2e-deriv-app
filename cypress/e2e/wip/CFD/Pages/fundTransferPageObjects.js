  ...
  mt5Deposit = (fiatAccount, mt5Account) => {
    ...
    cy.get('@fiatBalanceBefore').then((fiatBalanceBefore) => {
        ...
        if (fiatAccount == 'US Dollar') {
          ...
          cy.findByTestId(locators.fundTransferLocators.toAmountTestId)
            ...
            .then((convertedAmount) => {
              ...
              cy.get('@mt5BalanceAfter').then((mt5BalanceAfter) => {
                mt5BalanceAfter = Number(
                  mt5BalanceAfter.replace(/[^0-9.-]+/g, '')
                )
                expect(mt5BalanceAfter).to.eq(
                  mt5BalanceBefore + parseFloat(convertedAmount)
                )
              })
            })
        }
      ...
      cy.get('@fiatBalanceAfter').then((fiatBalanceAfter) => {
        ...
        expect(fiatBalanceAfter).to.eq(
          fiatBalanceBefore - parseInt(this.transferAmount)
        )
      })
    })
  }
      ...
      cy.get('@mt5BalanceBefore').then((mt5BalanceBefore) => {
        ...
        if (fiatAccount == 'US Dollar') {
          ...
          cy.get(locators.fundTransferLocators.fiatBalanceLabel)
            ...
            .as('fiatBalanceAfter')
          cy.get('@fiatBalanceAfter').then((fiatBalanceAfter) => {
            fiatBalanceAfter = Number(
              fiatBalanceAfter.replace(/[^0-9.-]+/g, '')
            )
            expect(fiatBalanceAfter).to.eq(
              fiatBalanceBefore + parseInt(this.transferAmount)
            )
          ...
          cy.findByTestId(locators.fundTransferLocators.toAmountTestId)
            ...
            .then((convertedAmount) => {
              ...
              cy.get('@fiatBalanceAfter').then((fiatBalanceAfter) => {
                ...
                expect(fiatBalanceAfter).to.eq(
                  Number(
                    (fiatBalanceBefore + parseFloat(convertedAmount)).toFixed(2)
                  )
                )
              })
            }) //convertedamount end
