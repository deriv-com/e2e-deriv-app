...
function navToMyCounterparties() {
    ...
    cy.findByText('When you block someone, you won\'t see their ads, and they can\'t see yours. Your ads will be hidden from their search results, too.').should('be.visible')
}

function createAnOrder(advertNickname) {
    cy.findAllByText(advertNickname).first().parentsUntil('.buy-sell-row').next().find('button').should('be.visible').click()
    cy.findByText('If the market rate changes from the rate shown here, we won\'t be able to process your order.').should('be.visible')
    cy.findByRole('button', { name: 'Confirm' }).should('exist').and('be.enabled').click()
    cy.findByText('Don\'t risk your funds with cash transactions. Use bank transfers or e-wallets instead.').should('be.visible')
...
function blockedAdvertiserValidation(advertNickname) {
    ...
    cy.findByPlaceholderText('Search by nickname').type(advertNickname).should('have.value', advertNickname)
    cy.findByText(advertNickname).parentsUntil('dc-table__cell').next().findByRole('button', { name: 'Unblock' }).should('be.visible')
    cy.findByTestId('dt_mobile_full_page_return_icon').should('exist').click()
    cy.findByText('Available Deriv P2P balance').should('be.visible')
    cy.findByText('Buy / Sell').click()
    cy.findByText(advertNickname).should('not.exist')
    cy.findByText('Orders').click()
    cy.get('.ReactVirtualized__Grid__innerScrollContainer').children().first().click()
    cy.findByText(advertNickname).should('be.visible')
...
function unblockProfile(advertNickname) {
    ...
    cy.findByText(`You will be able to see ${advertNickname}'s ads. They\'ll be able to place orders on your ads, too.`).should('be.visible')
    cy.findByText(`Unblock ${advertNickname}?`).should('be.visible')
    cy.findByTestId('dt_modal_footer').findByRole('button', { name: 'Unblock' }).should('be.enabled').click()
    cy.findByText('Advertiser\'s page').should('exist')
    cy.findByTestId('dt_page_return_icon').click()
    cy.findByText('My counterparties').should('be.visible')
    cy.findByTestId('dt_mobile_full_page_return_icon').should('exist').click()
...
function unblockedAdvertiserValidation(advertNickname) {
    ...
    cy.findByText('Please do not cancel if you have already made payment.').should('be.visible')
    cy.findByRole('button', { name: 'Cancel this order' }).should('be.enabled').click()
    cy.findByText(advertNickname).should('be.visible')
    cy.findByText('Cancelled').should('be.visible')
    cy.findByTestId('dt_mobile_full_page_return_icon').should('exist').click()
    navToMyCounterparties()
    cy.findByPlaceholderText('Search by nickname').type(advertNickname).should('have.value', advertNickname)
    cy.findByText(advertNickname).parentsUntil('dc-table__cell').next().findByRole('button', { name: 'Block' }).should('be.visible')
}
...
describe("QATEST-2871 - Block and unblock user from advertisers profile page", () => {
    ...
    it('Should be able to block and unblock the advertiser from profile page in responsive mode.', () => {
        ...
        cy.get('.buy-sell-row__advertiser-name--text').first().invoke('text').then((clientNickname) => {
