...
function clickAddMt5Button() {
  cy.get(
    '.wallets-available-mt5__details + .wallets-trading-account-card__content > .wallets-button'
  )
    .first()
    .click()
}

function verifyJurisdictionSelection(
  accountType,
...
function selectJurisdictionAndConfirm(
  accountType,
...
function verifyDerivMT5Creation(accountType) {
  ...
  cy.findByRole('button', {
    name:
      accountType === 'Derived' ? 'Create Deriv MT5 password' : 'Add account',
  }).click()
}
...
describe('QATEST-98638 - Add Real SVG MT5 account and QATEST-98818 Add demo SVG MT5 account and QATEST-115487 Add real BVI MT5 account', () => {
  ...
  it('should be able to create mt5 svg account', () => {
    ...
    if (getMoreText.length > 0) {
      ...
      cy.findByRole('button', { name: 'Next' }).click()
      selectJurisdictionAndConfirm('Derived')
      cy.findByRole('heading', {
        name: 'Complete your personal details',
      }).should('exist')
      cy.findByPlaceholderText('Tax residence*').click().type('Indonesi')
      cy.findByRole('option', { name: 'Indonesia' }).click()
      cy.findByLabelText('Tax identification number*')
        .click()
        .type('001234212343232')
      cy.findByRole('button', { name: 'Next' }).click()
    ...
    cy.contains('This account offers CFDs on derived instruments.').then(
      demoFinancialText
    )
  ...
  it.only('should be able to create mt5 svg account in responsive', () => {
    cy.log('create mt5 svg account')
    cy.findByText('CFDs', { exact: true }).should('be.visible')
